...
namespace FBXLoader
{
	...
	HRESULT LoadFBX(LPCSTR fileName,
		...
		std::vector<DWORD>* outIndices)
	{
		...
		if (fbxManager == NULL)
		{
			fbxManager = FbxManager::Create();  //fbx sdk 매니저 생성
	...
	XMFLOAT2 ReadUV(FbxMesh * fbxMesh, int controlPointIndex, int vertexCounter)
	{
		...
		switch (vertexUV->GetMappingMode())
		{
			...
			default:
			{
				MessageBox(NULL, TEXT("UV 값이 유효하지 않습니다."), TEXT("오류"), MB_OK);
				return XMFLOAT2();
			}
		}
