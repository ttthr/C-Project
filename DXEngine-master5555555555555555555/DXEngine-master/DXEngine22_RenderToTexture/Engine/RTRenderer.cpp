...
bool RTRenderer::Initiailze(ID3D11Device * pd3dDevice, ID3DBlob * vertexShaderBuffer, int screenWidth, int screenHeight, int bitmapWidth, int bitmapHeight)
{
	...
	//오류 확인
	if (IsError(result, TEXT("RT 상수버퍼 생성 실패")))
	{
		return false;
	}
...
void RTRenderer::UpdateBuffers(ID3D11DeviceContext * pd3dDeviceContext, int posX, int posY)
{
	...
	float top = static_cast<float>((screenHeight / 2)) - static_cast<float>(posY);
	...
	//버텍스 버퍼 업데이트
	pd3dDeviceContext->UpdateSubresource(vertexBuffer, NULL, NULL, &vertices, NULL, NULL);

}
void RTRenderer::RenderBuffers(ID3D11DeviceContext * pd3dDeviceCOntext)
{
	//바인딩
