...
namespace FBXLoader
{
	...
	XMFLOAT3 ReadTangent(FbxMesh * fbxMesh, int controlPointIndex, int vertexCounter)
	{
		...
		// UV 전체 배열 읽어오기.
		FbxGeometryElementTangent* vertexTagent = fbxMesh->GetLayer(0)->GetTangents();
		FbxLayerElement::EReferenceMode referenceMode = vertexTagent->GetReferenceMode();
		FbxLayerElement::EMappingMode mappingMode = vertexTagent->GetMappingMode();
		...
		if (mappingMode == FbxLayerElement::eByControlPoint)
		{
			if (referenceMode == FbxLayerElement::eDirect)
				...
				index = vertexTagent->GetIndexArray().GetAt(controlPointIndex);
		}
		else if (mappingMode == FbxLayerElement::eByPolygonVertex)
		{
			if (referenceMode == FbxLayerElement::eDirect)
				...
				index = vertexTagent->GetIndexArray().GetAt(vertexCounter);
		}

		tangent.x = static_cast<float>(vertexTagent->GetDirectArray().GetAt(index).mData[0]);
		tangent.y = static_cast<float>(vertexTagent->GetDirectArray().GetAt(index).mData[1]);
		tangent.z = static_cast<float>(vertexTagent->GetDirectArray().GetAt(index).mData[2]);
